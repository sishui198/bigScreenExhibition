...
function getITILData(){
  ...
  //       //console.log(ITILData.Last12Months[11-i]);
    ...
    $.ajax({
        ...
        url:'http://itsm.hxmis.com:8088/bigScreen/itil',
        type: 'get',
        // header:
        //dataType: 'jsonp',
        //data: {"TableID":"21"},
            ...
            if(response.StatusCode == 200){
                ...
                for(var i = 0 ;i < ITILData_Data.length ;i++){
                    ...
                    if(ITILData_Data[i].ID == '1'){
                        ...
                        for (var j = 0; j < ManageNormInfo_remote.length; j++) {
                            
                            ManageNormInfo_remote_zsArr.push((parseFloat(ManageNormInfo_remote[j].zs)*100).toFixed(2));
                        }

                        for (var j = 0; j < ManageNormInfo_widecity.length; j++) {
                            
                            ManageNormInfo_widecity_zsArr.push((parseFloat(ManageNormInfo_widecity[j].zs)*100).toFixed(2));
                        }
